      ...
      <section className="py-12 md:py-20" style={{ backgroundColor: "#111827" }}>
        <div className="max-w-7xl mx-auto px-4 md:px-6">
          ...
          <div className="max-w-md mx-auto space-y-4">
            {hours.map((row) => (
              <div key={row.day} className="flex justify-between py-3" style={{ borderBottom: "1px solid #374151" }}>
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-8">
            {contacts.map((c) => (
              <div key={c.title} className="rounded-2xl p-8 text-center" style={{ backgroundColor: "#F9FAFB", border: "1px solid #E5E7EB" }}>
                ...
                {c.href ? (
                  <a href={c.href} className="block font-semibold mb-1" style={{ color: "#DC2626", textDecoration: "none" }}>{c.value}</a>
                ) : (
                  <p className="font-semibold mb-1" style={{ color: "#DC2626" }}>{c.value}</p>
                )}
      ...
      <section className="py-12 md:py-20" style={{ backgroundColor: "#DC2626" }}>
        <div className="max-w-7xl mx-auto px-4 md:px-6 text-center">
          ...
          <div className="flex flex-col sm:flex-row gap-4 justify-center">
            <a href="mailto:office@baktransport.at" className="inline-block px-8 py-4 rounded-xl font-semibold text-base" style={{ backgroundColor: "#FFFFFF", color: "#DC2626" }}>{t.kontakt_cta_email_btn}</a>
            <a href="tel:+436761234567" className="inline-flex items-center justify-center gap-2 px-8 py-4 rounded-xl font-semibold text-base text-white" style={{ border: "2px solid rgba(255,255,255,0.6)" }}><Phone className="w-5 h-5" />{t.kontakt_cta_call_btn}</a>
          </div>
        </div>
      </section>
    </PageLayout>
