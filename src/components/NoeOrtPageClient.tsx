...
import { NOE_ORTE, type NoeOrt } from "@/data/noe-orte";

interface Props {
  ort: NoeOrt;
}

export function NoeOrtPageClient({ ort }: Props) {
  const { lang } = useLang();
  ...
  return (
    <PageLayout
      label={`${t.bezirk_label_prefix} ${ort.name}`}
      title={`${t.bezirk_label_prefix} ${ort.name}`}
      subtitle={`${t.noe_ort_subtitle_prefix} ${ort.name} ${t.bezirk_subtitle_suffix}`}
    >
      {/* Intro + Trust-Badges */}
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-12 items-start">
            <div className="flex flex-col items-center text-center">
              ...
              <h2 className="text-2xl md:text-3xl font-bold mb-5" style={{ color: "#9A3412" }}>
                {`${t.noe_ort_intro_h2_prefix} ${ort.name}`}
              </h2>
        ...
        <div className="max-w-3xl mx-auto px-4 md:px-6">
          <div className="text-center mb-10">
            <h2 className="text-2xl md:text-3xl font-bold mb-3" style={{ color: "#9A3412" }}>
              {`${t.noe_ort_pricing_h2_prefix} ${ort.name}`}
            </h2>
          ...
          <div className="rounded-2xl p-8 text-center" style={{ backgroundColor: "#FFFFFF", border: "1px solid #E5E7EB" }}>
            ...
            <div className="text-base mb-1" style={{ color: "#6B7280" }}>{t.noe_page_p1_detail}</div>
            <div className="text-3xl md:text-4xl font-black my-4" style={{ color: "#C2410C" }}>
              {t.noe_page_p1_price}
            </div>
            <a href="/anfrage"
              className="inline-block px-8 py-3 rounded-xl font-semibold text-sm text-center transition-opacity hover:opacity-90"
      ...
      {/* Ablauf */}
      <AblaufSection
        heading={`${t.bezirk_ablauf_heading_prefix} ${ort.name} ${t.bezirk_ablauf_heading_suffix}`}
        steps={[
          { title: t.bezirk_step1_title, text: t.bezirk_step1_text },
          { title: t.bezirk_step2_title, text: t.bezirk_step2_text },
          { title: t.bezirk_step3_title, text: t.noe_ort_step3_text },
      ...
      <section className="py-12 md:py-20" style={{ backgroundColor: "#F9FAFB" }}>
        <div className="max-w-4xl mx-auto px-4 md:px-6">
          <h2 className="text-2xl md:text-3xl font-bold mb-10" style={{ color: "#9A3412" }}>
            {`${t.bezirk_faq_h2_prefix} ${ort.name}`}
          </h2>
      ...
      <section className="py-12 md:py-20" style={{ backgroundColor: "#C2410C" }}>
        <div className="max-w-7xl mx-auto px-4 md:px-6 text-center">
          <h2 className="text-2xl md:text-3xl font-bold text-white mb-3">
            {`${t.bezirk_cta_h2_prefix} ${ort.name} ${t.bezirk_cta_h2_suffix}`}
          </h2>
